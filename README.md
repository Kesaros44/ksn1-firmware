...
- **Backlight:** wired and supported in firmware, but disabled by default (`CONFIG_ZMK_BACKLIGHT=n`)
- **RGB underglow:** none
...
- **`func_layer`** (hold `&mo 1`) — Bluetooth profile select (0–4) and clear, output toggle, backlight inc/dec on the encoder (no-op while backlight is disabled), toggle (`&tog 2`) into `mac_layer`.
...
- **백라이트:** 배선과 펌웨어 기능은 있지만 기본적으로 비활성화(`CONFIG_ZMK_BACKLIGHT=n`)
- **RGB 언더글로우:** 없음
- **배터리:** 양쪽 half 모두 보고
...
- **`func_layer`** (홀드 `&mo 1`) — 블루투스 프로필 선택(0–4) 및 clear, 출력 토글, 인코더로 백라이트 증감(백라이트가 비활성화라 실제 동작은 없음), `mac_layer`로의 토글(`&tog 2`).
